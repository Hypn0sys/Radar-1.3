...
class GLMap: InputAdapter(), ApplicationListener, GameListener {
  ...
  lateinit var smalllittleFont: BitmapFont
  ...
  override fun create() {
    ...
    param.color = GREEN
    param.size = 10
    smalllittleFont = generator.generateFont(param)
    param.color = BLACK
    param.size = 15
  ...
  override fun render() {
    ...
    for ((_, actor) in visualActors)
      ...
      paint(fontCamera.combined) {
          var strNumAliveTeams: String
          if (NumAlivePlayers == NumAliveTeams || NumAliveTeams == 0) {
              strNumAliveTeams = ""
          } else {
              strNumAliveTeams = "- " + NumAliveTeams.toString() + " teams"
          }
          smalllittleFont.draw(spriteBatch, "Optimized by HypnoSyS & RiBeLlu", 10f, windowHeight - 10f)
          littleFont.draw(spriteBatch, "Left : $NumAlivePlayers players $strNumAliveTeams\n" +
                  "Time elapsed : ${MatchElapsedMinutes} min\n", 10f, windowHeight - 35f)
    ...
    private fun ShapeRenderer.drawItem() {
        droppedItemLocation.values
                .forEach {
                    ...
                    val finalColor = when {
                        "m416" in items -> m416ItemColor
                        "scar" in items -> scarItemColor
                        "armor3" in items -> armor3ItemColor
                        "Extended" in items -> extendedItemColor
                        "Suppressor" in items -> suppressorItemColor
                        "Kar98k" in items || "kar98" in items || "Kar" in items -> kar98kItemColor
                        "mini" in items -> miniItemColor
                        "helmet3" in items -> helmet3ItemColor
                        "scope4x" in items -> scope4xItemColor
                        "scope8x" in items -> scope8xItemColor
                        "bag2" in items || "bag3" in items -> bagItemColor
                        "heal" in items || "drink" in items -> healItemColor
                        "grip" in items -> gripItemColor
                        "sks" in items -> sksItemColor
                        else -> normalItemColor
                    }

                    val epic = when (finalColor) {
                        kar98kItemColor, miniItemColor, sksItemColor -> true
                        else -> false
                    }
                    val rare = when (finalColor) {
                        m416ItemColor, scarItemColor, armor3ItemColor -> true
                        else -> false
                    }
                    val normal = when (finalColor) {
                        bagItemColor, healItemColor,  helmet3ItemColor -> true
                        else -> false
                    }
                    val access = when (finalColor) {
                        gripItemColor, extendedItemColor -> true
                        else -> false
                    }
                    val viseur = when (finalColor) {
                        scope4xItemColor, scope8xItemColor, suppressorItemColor -> true
                        else -> false
                    }
                    ...
                    when {
                        ...
                        normal -> {
                            color = BLACK
                            circle(x, y, backgroundRadius, 20)
                            color = finalColor
                        ...
                        access -> {
                            color = BLACK
                            rect(x - backgroundRadius, y - backgroundRadius, backgroundRadius * 0, backgroundRadius * 2)
                            color = finalColor
                            rect(x - radius, y - radius, radius * 1, radius * 2)
                        }
                        viseur -> {
                            color = BLACK
                            circle(x, y, backgroundRadius, 6)
                            color = finalColor
                            circle(x, y, radius, 6)
                        }
                        else -> { //nothing
  ...
  fun drawPlayerInfos(players: MutableList<renderInfo>?, selfX: Float, selfY: Float) {
    players?.forEach {
      ...
      nameFont.draw(spriteBatch, "$angle°${distance}", sx + 20, windowHeight - sy + 20)
    }
  }
